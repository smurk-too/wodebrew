...
int PNGU_DecodeToYCbYCr2 (IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, void *buffer, PNGU_u32 stride, PNGU_u32 width2, PNGU_u32 height2)
{
	int result;
	PNGU_u32 x, y, buffWidth;
	PNGUCOLOR buff[2048];
	...
	//buff = malloc ((sizeof(PNGUCOLOR) + 1) * width2 * 2);

	//if(buff)
	//{
		short factorX = (width2 << 8) / width;
		short factorY = (height2 << 8) / height;
		short restX, restY, rest, factor;
		PNGUCOLOR c1, c;
		register PNGUCOLOR *source, *dest;

		PNGUCOLOR *prevLine, *currLine;
		
		currLine = buff + (sizeof(PNGUCOLOR) + 1) * width2;

		PNGU_u32 *screen = buffer;

		memset(buff, '\0', (sizeof(PNGUCOLOR) + 1) * width2);
		prevLine = buff;
		restY = 0;
		...
		{
			// calculate one row
			source = (PNGUCOLOR *)(ctx->row_pointers[y]);
			dest = currLine;
			c1.r = c1.g = c1.b = 0;
			restX = 0;
			for (x = 0; x < width; x++)
			{
				...
				if (restX > 0)
				{
					rest = restX > factor ? factor : restX;
					c1.r += (c.r * rest) >> 8;
					c1.g += (c.g * rest) >> 8;
					c1.b += (c.b * rest) >> 8;
					factor -= rest;
			...
			if (restY > 0)
			{
				rest = restY > factor ? factor : restY;
				for (source = currLine, dest = prevLine, x = 0; x < width2; x++, source++, dest++)
				{
					dest->r += (source->r * rest) >> 8;
					dest->g += (source->g * rest) >> 8;
					dest->b += (source->b * rest) >> 8;
				}
				factor -= rest;
				restY -= rest;
				if( restY <= 0)
				{
					for (source = prevLine, dest = source + 1, x = 0; x < (width2 / 2); x++, source += 2, dest += 2)
						*screen++ = PNGU_RGB8_TO_YCbYCr (source->r, source->g, source->b, dest->r, dest->g, dest->b);
					screen += buffWidth - (width2 / 2);
					memset(prevLine, '\0', (sizeof(PNGUCOLOR) + 1) * width2);
				}
			}
			while( factor >= 256)
			{
				for (source = currLine, dest = source + 1, x = 0; x < (width2 / 2); x++, source += 2, dest += 2)
					*screen++ = PNGU_RGB8_TO_YCbYCr (source->r, source->g, source->b, dest->r, dest->g, dest->b);
				screen += buffWidth - (width2 / 2);
				factor -= 256;
			}
			if (factor > 0)
			{
				for (source = currLine, dest = prevLine, x = 0; x < width2; x++, source++, dest++)
				{
					dest->r = (source->r * factor) >> 8;
					dest->g = (source->g * factor) >> 8;
					dest->b = (source->b * factor) >> 8;
				}
				restY = 256 - factor;
			}
		}
...
int PNGU_DecodeToYCbYCrFuchsiaTrans(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, void *buffer, PNGU_u32 stride)
{
	int result;
	PNGU_u32 x, y, buffWidth;
	...
	for (y = 0; y < height; y++)
		for (x = 0; x < (width / 2); x++){
			u32 color=PNGU_RGB8_TO_YCbYCr (*(ctx->row_pointers[y]+x*6), *(ctx->row_pointers[y]+x*6+1), *(ctx->row_pointers[y]+x*6+2),*(ctx->row_pointers[y]+x*6+3), *(ctx->row_pointers[y]+x*6+4), *(ctx->row_pointers[y]+x*6+5));
			if(color!=0x69D469EA) //0x69D469EA==FUCHSIA
				((PNGU_u32 *)buffer)[y*buffWidth+x] = color;
		}
