...
//    Settings.godmode = 1;
	...
	Settings.partition = WBFS_GetDefaultPartition();
...
bool cfg_load_games() {
    ...
    sprintf(GXGameSettings_cfg, "%s/config/WGXGameSettings.cfg", bootDevice);
    return cfg_parsefile(GXGameSettings_cfg, &game_set);
}

bool cfg_load_game_num() {
    char GXGameFavorites_cfg[32];
    sprintf(GXGameFavorites_cfg, "%s/config/WGXGameFavorites.cfg", bootDevice);
...
//	chdir_app(argv[0]);
    ...
    snprintf(pathname, sizeof(pathname), "%s/config/WGXGlobal.cfg", bootDevice);
	...
	CLEAR_wCOORD_FLAGS;
	snprintf(pathname, sizeof(pathname), "%sWGXtheme.cfg", CFG.theme_path);
...
//	cfg_parsefile(pathname, &language_set);
    ...
    snprintf(pathname, sizeof(pathname), "%s/config/WGXGameSettings.cfg", bootDevice);
