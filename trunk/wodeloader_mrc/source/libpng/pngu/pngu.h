...
#define PNGU_DECODE_TO_COORDS_YCbYCr2(ctx,coordX,coordY,imgWidth,imgHeight,bufferWidth,bufferHeight,buffer,imgWidth2,imgHeight2)	\
		PNGU_DecodeToYCbYCr2 (ctx, imgWidth, imgHeight, ((void *) buffer) + (coordY) * (bufferWidth) * 2 +	\
							(coordX) * 2, (bufferWidth) - (imgWidth), imgWidth2, imgHeight2)

//hace que el fucsia sea transparente
int PNGU_DecodeToYCbYCrFuchsiaTrans(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, void *buffer, PNGU_u32 stride);
#define PNGU_DECODE_TO_COORDS_YCbYCrTrans(ctx,coordX,coordY,imgWidth,imgHeight,bufferWidth,bufferHeight,buffer)	\
																											\
		PNGU_DecodeToYCbYCrFuchsiaTrans(ctx, imgWidth, imgHeight, ((void *) buffer) + (coordY) * (bufferWidth) * 2 +	\
