					...
					while (!exit)
					{
						...
						if(firstRun || ret >= 0)
						{
							...
							if (ret == ++Idx || firstRun)
							{
								...
								if (ret == Idx) {
									Settings.partition = Settings.partition + 1 == (u8) WBFS_GetPartitionCount() ? 0 : Settings.partition + 1;
								}
								
								char partName[32];
								memset((char *) &partName, 0, 32);
								WBFS_GetPartitionName(Settings.partition, (char *) &partName);
								
								// Get the partition name and it's size in GB's
								options2.SetValue(Idx,"%s",	partName);
							}
