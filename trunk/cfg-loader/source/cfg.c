...
char FAT_DRIVE[8] = "sd:";
char USBLOADER_PATH[200] = "sd:/wode-loader";
char APPS_DIR[200] = "";
...
bool is_gameid(char *id)
{
	int i;
	for (i=0; i<6; i++) {
		if (!isalnum((int) id[i])) return false;
		if (isalpha((int) id[i]) && islower((int) id[i])) return false;
	}
	return true;
}
...
void CFG_Partition_Default()
{
	if (CFG.partition[0] == '\0') {
		u32 defaultPartition = WBFS_GetDefaultPartition();
		WBFS_GetPartitionName(defaultPartition, CFG.partition);
	}
}
...
char* trimcopy_n(char *dest, char *src, int n, int size)
{
	...
	while (isspace((int) *src) && n>0) { src++; n--; }
	len = strlen(src);
	if (len > n) len = n;
	// trim trailing white space
	while (len > 0 && isspace((int) src[len-1])) len--;
	// limit length
...
void CFG_Load(int argc, char **argv)
{
	...
	snprintf(D_S(USBLOADER_PATH), "%s%s", FAT_DRIVE, "/wode-loader");
